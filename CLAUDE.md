...
- **Parsing FSE — pièges résolus** (dans `run_job_fse.py::_parse_and_save_fse`, chemin actif GitHub Actions, et son miroir `main.py::_parse_fse_bank_sync`) :
  1. **Off-by-one d'en-tête (cause RÉELLE du virement Biogaran 1 396,29 € du 29/10/25 « introuvable »)** : le parcours démarrait à `hdr_idx + 2` au lieu de `hdr_idx + 1` → la PREMIÈRE ligne de données de CHAQUE export mensuel était sautée. Le virement (première ligne du fichier d'octobre, libellé normal « VIR BIOGARAN - 9006187702 … ») n'était donc jamais parsé — ce n'était NI un problème d'export Webix NI de préfixe de libellé. Symptôme de diagnostic : total parsé ≠ somme des lignes visibles (« 2 virements · 6 828 € » = 6 540 + 288, sans le 1 396,29).
  2. **Libellés sans préfixe « VIR »** : une ligne non-« VIR » est quand même gardée si elle passe l'un des 3 filets : (1) libellé contenant un n° de facture/avoir Digi connu ; (2) montant À DÉCIMALES = un avoir Digi au centime ; (3) CRÉDIT à décimales nommant un labo génériqueur connu (`_identify_labo` ≠ None). Décimales exigées → collision quasi impossible ; sommes RONDES (coop) exclues du filet 3 (exigent un n° de document).
  3. **Intermédiaires de paiement (Pharmedigroupe → Zydus & Mylan/Viatris)** : `_identify_labo` cherche un labo réel dans le bénéficiaire PUIS dans le libellé entier, sinon garde la ligne sous « PHARMEDI » (`_PAYER_KEYS`). Côté front, `_FSE_PAYERS` (payeur → labos) rend ces virements candidats pour Zydus/Viatris dans `_fseAssignMap`, mais attribution UNIQUEMENT par n° de document (avoir/facture_refs) — jamais en repli mois d'encaissement (même payeur = plusieurs labos → pas d'attribution au hasard).
  4. **`serializeState` émet `fse_month_stats` (snake) EN PLUS de `fseMonthStats`** : `saveCloudState` remplace tout `state_json` → sans la clé snake, chaque sauvegarde front effaçait la sortie du scraper. Le refresh « concentrateur » préfère aussi le snake (source de vérité backend).
