...
- **Parsing FSE — pièges résolus** (dans `run_job_fse.py::_parse_and_save_fse`, chemin actif GitHub Actions, et son miroir `main.py::_parse_fse_bank_sync`) :
  ...
  3. **`serializeState` émet `fse_month_stats` (snake) EN PLUS de `fseMonthStats`** : `saveCloudState` remplace tout `state_json` → sans la clé snake, chaque sauvegarde front effaçait la sortie du scraper. Le refresh « concentrateur » préfère aussi le snake (source de vérité backend).
