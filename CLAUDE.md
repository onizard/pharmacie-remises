...
- **Parsing FSE — pièges résolus** (dans `run_job_fse.py::_parse_and_save_fse`, chemin actif GitHub Actions, et son miroir `main.py::_parse_fse_bank_sync`) :
  ...
  3. **Intermédiaires de paiement (Pharmadigroupe → Zydus & Mylan/Viatris)** : `_identify_labo` cherche un labo réel dans le bénéficiaire PUIS dans le libellé entier, sinon garde la ligne sous « PHARMADI » (`_PAYER_KEYS`). Côté front, `_FSE_PAYERS` (payeur → labos) rend ces virements candidats pour Zydus/Viatris dans `_fseAssignMap`, mais attribution UNIQUEMENT par n° de document (avoir/facture_refs) — jamais en repli mois d'encaissement (même payeur = plusieurs labos → pas d'attribution au hasard).
